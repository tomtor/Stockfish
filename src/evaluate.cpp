...
namespace {
  ...
  // Rank factor applied on some bonus for passed pawn on rank 3 or beyond
  const int RankFactor[RANK_NB] = {0, 0, 1, 3, 8, 13, 20};
    ...
    while (b)
    {
        ...
        if (rr)
        {
            ...
            if (!(attackedBy[Us][ALL_PIECES] & s))
                rr = std::max(0, rr - (pos.side_to_move() == Them) - popcount(pe->passed_pawns(Us)));
